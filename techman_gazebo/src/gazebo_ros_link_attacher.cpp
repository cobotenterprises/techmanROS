...
#include "techman_gazebo/Attach.h"
#include "techman_gazebo/AttachRequest.h"
#include "techman_gazebo/AttachResponse.h"
...
namespace gazebo
{
                                     ...
                                     std::string model2, std::string link2)
  {
    ...
    // ROS_DEBUG_STREAM("Creating fixed joint on model: '" << model1 << "'");
    j.joint = this->physics->CreateJoint("fixed", m1);
    this->joints.push_back(j);

    // ROS_DEBUG_STREAM("Attach");
    j.joint->SetAnchor(0, ignition::math::Vector3(0.0, 0.0, 0.0));
    j.joint->SetAxis(0, ignition::math::Vector3(0.0, 0.0, 0.0));
    j.joint->Attach(l1, l2);
    // ROS_DEBUG_STREAM("Loading links");
    j.joint->Load(l1, l2, ignition::math::Pose3d(0, 0, 0, 0, 0, 0));
  ...
  bool GazeboRosLinkAttacher::attach_callback(techman_gazebo::Attach::Request &req,
                                              techman_gazebo::Attach::Response &res)
  {
  ...
  bool GazeboRosLinkAttacher::detach_callback(techman_gazebo::Attach::Request &req,
                                              techman_gazebo::Attach::Response &res){
